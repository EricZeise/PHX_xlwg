# PHPP Field Mapping Reference — EN_10_6_SI

Localization map for **PHPP 10.6 (English, SI units)**. Targets a genuinely SI-native PHPP workbook (single-shell, no `<Name> SI`-suffixed mirror tabs) — verified against `Data/Example_SI.xlsx`/`Data/Empty_SI.xlsx`. Sheet names are used exactly as declared here; there is no runtime SI/IP sheet-name guessing.
...
| `setpoint_winter` (input) | Interior temperature winter [°C]: | J | K | 0 | C |  |
| `setpoint_summer` (input) | Interior temp. summer [°C]: | M | N | 0 | C |  |
